import { LOAD_USER, LOGOUT_SUCCESS, LOGIN_SUCCESS } from './types';
...
export const signIn = payload => {
  return async dispatch => {
    ...
    if (!payload.email || !payload.name || !payload.password) {
      dispatch(
        showAlert({ msg: 'Please complete all the fields', color: 'black' })
      );
  ...
  return async dispatch => {
    ...
    if (res.status === 200) {
      dispatch(
        loadUser({
          token: data.token,
          keepUserLoggedIn: payload.keepUserLoggedIn
        })
      );
    } else {
      dispatch(showAlert({ msg: data.msg, color: 'primary' }));
